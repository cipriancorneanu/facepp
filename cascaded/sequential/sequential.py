...
class Sequential:
    ...
    def train(self, sequences, geometries, save_file=None):
        ...
        # If save file provided, save model
        self.save(save_file)
    ...
    def test(self, sequences, geometries):
        ...
        # Initialize method-specific memory
        ...
        for i in range(1, max(seq_lens)):
            ...
            frames, unfinished = self._select_not_lost(frames, unfinished)
    ...
    def _select_not_lost(self, frames, unfinished):
        in_image = [(not ((
            np.min(g[:, 0]) / f.shape[0] > 1
        ) or (
            np.max(g[:, 0]) / f.shape[0] < 0
        ) or (
            np.min(g[:, 1]) / f.shape[1] > 1
        ) or (
            np.max(g[:, 1]) / f.shape[1] < 0)
        )) for f, g in zip(frames, self.geometries[unfinished])]

        return (
            [frames[i] for i, v in enumerate(in_image) if v],
            [unfinished[i] for i, v in enumerate(in_image) if v]
        )

...
class DescriptorSiftRotate(Descriptor):
    ...
    def _extract(self, images, coords, mapping, args):
        ...
        # Prepare descriptors
        descriptors = np.zeros(tuple(coords.shape[:2]) + (4*4, nb), dtype=np.float32)
        ...
        for i, (c, r, mp, ws) in enumerate(zip(coords, rotations, mapping, win_sizes)):
            ...
            for j, (jc, jv) in enumerate(zip(c, vis[i, :])):
                if jv:
                    ...
                    descriptors[i, j, :, :] = [np.bincount(b, weights=m, minlength=nb) for m, b in zip(t_mag, t_bin)]

            # Normalize image features
            descriptors[i, ...] /= np.maximum(np.sum(descriptors[i, ...], axis=2, keepdims=True), 0.00001)
